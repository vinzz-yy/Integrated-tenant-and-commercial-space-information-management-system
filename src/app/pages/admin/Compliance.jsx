...
import { Search, Plus, ClipboardList, Clock, CheckCircle, XCircle, Pencil, Trash2, MoreVertical, Eye } from 'lucide-react';
...
export function Compliance() {
  ...
  useEffect(() => {
    ...
    const refreshRequests = async ({ silent = false } = {}) => {
      try {
        ...
        if (!silent) {
          ...
          // avoid noisy popups during background refresh
    ...
    loadUsersOnce();
    refreshRequests({ silent: false }).finally(() => {
      didInitialLoadRef.current = true;
    });

    // Near real-time: refresh requests periodically
